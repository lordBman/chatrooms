...
export interface SubChatDetails{
    id: number,
    message: string,
    user: UserDetails,
}
...
export interface ChatDetails{
    ...
    message: string,
    user: UserDetails,
    reply: SubChatDetails[]
}

export default class Chat {    
    static async details(id: number): Promise<ChatDetails | undefined>{
        try{
            const result = await DBManager.instance().db.chat.findFirst({ where: { id },
                include:{
                    user: { include: { profile: true } },
                    reply: { include: { user: { include: { profile:  true } } } }
                } })
            if(result){
                return result;
            }
    ...
    static async get(roomID: number, page: number = 1): Promise<ChatDetails[] | undefined>{
        const commentsPerPage = 10; // Number of users to display per page
        try{
            const result = await DBManager.instance().db.chat.findMany({
